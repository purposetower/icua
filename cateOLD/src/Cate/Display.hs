module Cate.Display(moveToEnd, createDisplay, displayEditor, insertIntoEditor, deleteFromEditor,
...
moveToEnd editor@(Editor _ oldBuffer@(Buffer cursors bufferStartPosition bufferData) _) =
    editor {buffer = moveBufferInsertPosition (length bufferData) oldBuffer}
...
moveStartPositionToShowLastCursor editor@(Editor _ oldBuffer@(Buffer cursors bufferStartPosition bufferData)
    ...
    where
        ...
        bufferSections = calculateBufferSectionsWrap 0 width bufferData
...
getCursorPositionFromBufferSections ((SimpleCursor cursorPosition):xs) startPosition terminalWindowSize@(TerminalWindowSize width height) bufferData
    ...
    where
        bufferSections = take height (calculateBufferSectionsWrap startPosition width (drop startPosition bufferData))
...
getCursorPositionFromBufferSections ((SelectionCursor cursorStartPosition cursorEndPosition):xs) sectionStartPosition terminalWindowSize@(TerminalWindowSize width height) bufferData
    ...
    where
        bufferSections = calculateBufferSectionsWrap sectionStartPosition width bufferData
...
getDisplayFromBufferSections sectionStartPosition (TerminalWindowSize width height) bufferData
    ...
    where
        bufferSections = take height (calculateBufferSectionsWrap sectionStartPosition width (drop sectionStartPosition bufferData))
