...
getDisplayText input (DisplaySize width height) wrapMode padded
    ...
    where
        ...
        emptyLines = foldr (++) "" $ take ((fromIntegral height) - length lines) $ repeat $ linePadding width
        -- init to remove last new line
        emptyLinesRemoveLastNewLine = if emptyLines /= [] then init emptyLines else []

        result = if padded then concatLines lines ++ emptyLinesRemoveLastNewLine else concatLines lines
...
getLinesNoWrap input width leftMarginDisplayOffset padded =
    ...
    where
        inputAfterLeftMarginApplied = drop (fromIntegral leftMarginDisplayOffset) input
